    ...
    public class References : Base
    {
        ...
        public void GenerateTagTemplateXml()
        {
            ...
            xml = Regex.Replace(xml, "<!DOCTYPE [^>]*>", "");
            {
                ...
                fp.Xml = XsltOnString.ApplyTransform(config.referencesGetReferencesXslPath, xml);
                fp.WriteXMLFile();
            }
            {
                // References template
                fp.OutputFileName = config.referencesTagTemplateXmlPath;
                fp.Xml = XsltOnString.ApplyTransform(config.referencesTagTemplateXslPath, xml);
        ...
        public void TagReferences()
        {
                ...
                for (Match m = Regex.Match(xml, @"<xref ref-type=""bibr""[^>]+>[^<>]+(?<=[^\)\]])</xref>(\W\s*\b(\d{2,4}(\W\d{1,4})?[a-z]?|[a-z])\b)*\W\s*\b\d{2,4}(\W\d{1,4})?[a-z]?\b"); m.Success; m = m.NextMatch())
                {
                    ...
                    for (Match l = Regex.Match(m.Value, @"(?<=<xref [^>]+>[^<>]*</xref>(\W\s*\b(\d{2,4}(\W\d{1,4})?[a-z]?|[a-z])\b)*\W\s*)\b\d{2,4}(\W\d{1,4})?[a-z]?\b"); l.Success; l = l.NextMatch())
                    {
                        try
                        {
                            ...
                            if (node != null)
                            {
                                replace = Regex.Replace(replace, "(?<=<xref [^>]+>[^<>]*</xref>(\\W\\s*\\b(\\d{2,4}(\\W\\d{1,4})?[a-z]?|[a-z])\\b)*\\W\\s*)\\b" + l.Value + "\\b",
            ...
            for (Match m = Regex.Match(xml, @"<xref ref-type=""bibr"" [^>]+>[^<>]*\d+\W*[a-z]\W*</xref>(\W\s*(\b[a-z]\b))+"); m.Success; m = m.NextMatch())
            {
                string rid = Regex.Replace(m.Value, @"\A.*<xref [^<>]*rid=""(\w+)""[^<>]*>.*\Z", "$1");
                string authors = referenceList.SelectSingleNode("//reference[@id='" + rid + "']/@authors", namespaceManager).InnerText;
                string year = Regex.Replace(referenceList.SelectSingleNode("//reference[@id='" + rid + "']/@year", namespaceManager).InnerText, "[A-Za-z]", "");
                ...
                for (Match l = Regex.Match(m.Value, @"(?<=<xref [^>]+>[^<>]*</xref>(\W\s*\b[a-z]\b)*\W\s*)\b[a-z]\b"); l.Success; l = l.NextMatch())
                {
                    try
                    {
                        ...
                        if (node != null)
                        {
                            replace = Regex.Replace(replace, "(?<=<xref [^>]+>[^<>]*</xref>(\\W\\s*\\b[a-z]\\b)*\\W\\s*)\\b" + l.Value + "\\b",
                                "<xref ref-type=\"bibr\" rid=\"" + node.Attributes["id"].Value + "\">" + l.Value + "</xref>");
                        }
                    }
                    catch (Exception e)
             ...
             */
            {
                ...
                while (xrefInRef)
                {
                    nodeList = xmlDocument.SelectNodes("//ref[.//xref[@ref-type='bibr']]", namespaceManager);
                    if (nodeList.Count > 0)
                    {
                        foreach (XmlNode node in nodeList)
                        {
                            node.InnerXml = Regex.Replace(node.InnerXml, @"<xref[^<>]+ref-type=""bibr""[^<>/]*>([^<>]*)</xref>", "$1");
                            node.InnerXml = Regex.Replace(node.InnerXml, @"<xref[^<>]+ref-type=""bibr""[^<>/]*/>", "");
                        }
                    }
                    else
                    {
        ...
        public void SplitReferences()
        {
            ...
            try
            {
                XmlNodeList nodeList = xmlDocument.SelectNodes("//element-citation|//mixed-citation|nlm-citation", namespaceManager);
                foreach (XmlNode node in nodeList)
                {
        ...
        public static string ReferencePartSplitter(XmlNode reference)
        {
            ...
            result = Regex.Replace(result, @"(?i)\s*(((\[\s*in.*?\]|\bdoi\W\s*\S+|\bhttps?\S+|\[\s*access.*?\]|\(\s*access.*?\))[,;\.\s]*)+)(</fake_tag>)", "$4 <fake_tag_1>$1</fake_tag_1>");
        ...
        public static string ReferenceJournalMatch(XmlNode reference)
        {
            ...
            reference.InnerXml = Regex.Replace(reference.InnerXml, "</?fake_tag_1>", "");
            ...
            reference.InnerXml = Regex.Replace(reference.InnerXml, "</?fake_tag>", "");
