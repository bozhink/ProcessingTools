namespace ProcessingTools.Journals.Data.Entity.Abstractions
{
    ...
    public abstract class AbstractEntityAddressableRepository<TEntity, TDbModel> : AbstractEntityRepository<TEntity, JournalsDbContext, TDbModel>
        where TEntity : class, IAddressable
        where TDbModel : Addressable, TEntity, ProcessingTools.Models.Contracts.IStringIdentifiable
    {
        protected AbstractEntityAddressableRepository(IEfRepository<JournalsDbContext, TDbModel> repository)
            : base(repository)
        {
        }
