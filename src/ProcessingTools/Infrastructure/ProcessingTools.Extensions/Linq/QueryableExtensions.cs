...
namespace ProcessingTools.Extensions.Linq
{
    ...
    public static class QueryableExtensions
    {
        ...
        public static IOrderedQueryable<T> OrderBySortExpression<T>(this IQueryable<T> source, string sortExpression, string defaultSort = "ID")
        {
            ...
            {
                sortExp = sortExpression.Split(',');
            }
            else
            {
                sortExp = new string[] { sortExpression };
            }

            object result = source;

            for (int i = 0; i < sortExp.Length; i++)
            {
                string[] split = sortExp[i].Split(' ');
                string sort = split[0];
                bool descending = split.Length > 1 && split[1].ToUpperInvariant().StartsWith("DESCENDING");
                if (i == 0)
                {
        ...
        {
            string[] filterExpressionValues = filterExpression?.Split('=') ?? new string[] { };
