        ...
        public class Tagger : Base
        {
            ...
            public static string TagItalics(string nodeXml, bool tagInfraspecific = false)
            {
                ...
                if (tagInfraspecific)
                {
                    ...
                    infraspecificPattern = @"<i(talic)?><tn type=""lower"">([^<>]*?)</tn></i(talic)?>(?![,\.])\s*(([^<>\(\)\[\]]{0,3}?\([^<>\(\)\[\]]{0,30}?\)[^<>\(\)\[\]]{0,30}?|[^<>\(\)\[\]]{0,10})?)\s*((\b([Aa]b?|[Ss]p|[Vv]ar|[Ss]ubvar|[Ss]ubsp|[Ss]ubspecies|[Ss]sp|f|[Ff]orma?|[Ss]f|[Cc]f|[Nn]r|sp\. near|[Aa]ff|[Pp]rope|([Ss]ub)?[Ss]ect)\b(\.)?)|×|\?)\s*<i(talic)?>([a-z-]+)</i(talic)?>";
                    for (Match m = Regex.Match(replace, infraspecificPattern); m.Success; m = m.NextMatch())
                    {
                        replace = Regex.Replace(replace, infraspecificPattern,
                    ...
                    infraspecificPattern = @"(?<=</infraspecific>)</tn>\s*([^<>]{0,100}?)\s*((\b([Aa]b?|[Nn]?\.?\s*[Ss]p|[Vv]ar|[Ss]ubsp|[Ss]ubspecies|[Ss]sp|[Ss]ubspec|f|fo|[Ff]orma?|[Ss]f|[Cc]f|[Nn]r|[Aa]ff|[Pp]rope|([Ss]ub)?[Ss]ect)\b(\.)?)|×)\s*<i(talic)?>([a-z-]+)</i(talic)?>";
                    for (int i = 0; i < 4; i++)
                    {
                        for (Match m = Regex.Match(replace, infraspecificPattern); m.Success; m = m.NextMatch())
                        {
            ...
            public void ExtractTaxaToReplaceQuery(string outputFileName)
            {
                FileProcessor fp = new FileProcessor();
            ...
            public void TagHigherTaxa()
            {
                ...
                try
                {
                    ...
                    if (config.NlmStyle)
                    {
                        foreach (XmlNode node in xmlDocument.SelectNodes("//tp:nomenclature-citation|//tp:Taxon-name", namespaceManager))
                        {
                            replace = Regex.Replace(node.InnerXml, searchPattern, higherTaxaReplacePattern);
                            replace = Regex.Replace(replace, "(<[^<>]*)<[^>]*>([^<>]*)</[^>]*>([^>]*>)", "$1$2$3");
                            node.InnerXml = replace;
            ...
            public void ParseTreatmentMetaWithAphia()
            {
                ...
                try
                {
                    List<string> genusList = xmlDocument.SelectNodes("//tp:taxon-treatment[string(tp:treatment-meta/kwd-group/kwd/named-content[@content-type='order'])='ORDO' or string(tp:treatment-meta/kwd-group/kwd/named-content[@content-type='family'])='FAMILIA']/tp:nomenclature/tp:taxon-name/tp:taxon-name-part[@taxon-name-part-type='genus']", namespaceManager).Cast<XmlNode>().Select(c => c.InnerXml).Distinct().ToList();
                    ...
                    foreach (string genus in genusList)
                    {
                        ...
                        List<string> responseFamily = response.SelectNodes("//return/item[string(genus)='" + genus + "']/family", namespaceManager).Cast<XmlNode>().Select(c => c.InnerXml).Distinct().ToList();
                        List<string> responseOrder = response.SelectNodes("//return/item[string(genus)='" + genus + "']/order", namespaceManager).Cast<XmlNode>().Select(c => c.InnerXml).Distinct().ToList();
                        List<string> responseKingdom = response.SelectNodes("//return/item[string(genus)='" + genus + "']/kingdom", namespaceManager).Cast<XmlNode>().Select(c => c.InnerXml).Distinct().ToList();
            ...
            public void ParseTreatmentMetaWithCoL()
            {
                ...
                try
                {
                    ...
                    foreach (string genus in genusList)
                    {
                        ...
                        if (response != null)
                        {
                            List<string> responseFamily = response.SelectNodes("/results/result[string(name)='" + genus + "']/classification/taxon[string(rank)='Family']/name", namespaceManager).Cast<XmlNode>().Select(c => c.InnerXml).Distinct().ToList();
                            List<string> responseOrder = response.SelectNodes("/results/result[string(name)='" + genus + "']/classification/taxon[string(rank)='Order']/name", namespaceManager).Cast<XmlNode>().Select(c => c.InnerXml).Distinct().ToList();
                            List<string> responseKingdom = response.SelectNodes("/results/result[string(name)='" + genus + "']/classification/taxon[string(rank)='Kingdom']/name", namespaceManager).Cast<XmlNode>().Select(c => c.InnerXml).Distinct().ToList();
            ...
            public void PerformFloraReplace(string xmlTemplate)
            {
                ...
                for (int i = root.ChildNodes.Count - 1; i >= 0; i--)
                {
                    ...
                    //Alert.Message(Regex.Escape(pattern));
                    //Alert.Message();

                    //for (Match m = Regex.Match(xml, "(?<![a-z-])(?<!<tn>)(" + pattern + ")(?![A-Za-z])(?!</tn\\W)(?!</tp:)(?!</name>)"); m.Success; m = m.NextMatch() )
                    //{
