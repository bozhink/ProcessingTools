namespace ProcessingTools.Configuration.Tests.IntegrationTests
{
    ...
    public class ConfigurationModelTests
    {
        ...
        [Test(Description = "ConfigurationModel: deserialization of a valid configuration JSON file as IConfigurationModel should work.", Author = "Bozhin Karaivanov", TestOf = typeof(ConfigurationModel))]
        public void ConfigurationModel_DeserializationOfValidConfigurationJsonFileAsIConfigurationModel_ShouldWork()
        {
            var fileName = "configuration.json";
            var directory = this.GetDirectoryOfTestAssembly;
            var filePath = Path.Combine(directory, Path.Combine("Files", fileName));
            ...
            var obj = JsonConvert.DeserializeObject<ConfigurationModel>(json);
            Assert.IsNotNull(obj, "De-serialized model object should not be null.");
            Assert.IsInstanceOf<IConfigurationModel>(obj, $"De-serialized model object should be instance of {typeof(IConfigurationModel).FullName}.");

            var model = obj as IConfigurationModel;

            const int NumberOfFileEntries = 2;
            const int NumberOfSettingEntries = 1;

            Assert.AreEqual(NumberOfFileEntries, model.Files.Count(), $"Number of file entries should be {NumberOfFileEntries}");
            Assert.AreEqual(NumberOfSettingEntries, model.Settings.Count(), $"Number of setting entries should be {NumberOfSettingEntries}");

            Assert.AreEqual("NlmInitialFormatXslPath", model.Files.ElementAt(0).Key, "Invalid Key-value in the first file object.");
            Assert.AreEqual("Xsl/format.nlm.initial.xsl", model.Files.ElementAt(0).Value, "Invalid Value-value in the first file object.");

            Assert.AreEqual("SystemInitialFormatXslPath", model.Files.ElementAt(1).Key, "Invalid Key-value in the second file object.");
            Assert.AreEqual("Xsl/format.system.initial.xsl", model.Files.ElementAt(1).Value, "Invalid Value-value in the second file object.");

            Assert.AreEqual("RedisServerAddress", model.Settings.ElementAt(0).Key, "Invalid Key-value in the setting object.");
            Assert.AreEqual("redis://localhost:6379", model.Settings.ElementAt(0).Value, "Invalid Value-value in the setting object.");
        }
    }
}
