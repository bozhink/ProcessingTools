namespace ProcessingTools.SystemCommons
{
    ...
    public class Net
    {
        ...
        {
            var response = await Net.GetStringAsync(url);
        ...
        public static async Task<string> PostStringAsync(string url, string content, string contentType = DefaultContentType)
        {
            try
            {
                byte[] bytes = Net.DefaultEncoding.GetBytes(content);
        ...
        {
            using (var client = new HttpClient())
            using (HttpContent content = new WeakFormUrlEncodedContent(values, Net.DefaultEncoding))
            {
        ...
        {
            var response = await Net.PostUrlEncodedStringAsync(url, values, encoding);
