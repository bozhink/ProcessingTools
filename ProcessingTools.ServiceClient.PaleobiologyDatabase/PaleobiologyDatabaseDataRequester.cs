    ...
    public class PaleobiologyDatabaseDataRequester
    {
        ...
        public static async Task<string> SearchNameInPaleobiologyDatabase(string scientificName)
        {
            ...
            try
            {
                ...
                {
                    string responseString = await client.GetStringAsync("https://paleobiodb.org/data1.1/taxa/single.txt?name=" + scientificName);
        ...
        public static async Task<PbdbAllParents> SearchParentsInPaleobiologyDatabase(string scientificName)
        {
            try
            {
                using (var client = new HttpClient())
                {
                    var response = await client.GetStringAsync("https://paleobiodb.org/data1.1/taxa/list.json?name=" + scientificName + "&rel=all_parents");
